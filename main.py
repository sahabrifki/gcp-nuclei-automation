...
def run_nuclei():    
    path_to_file = "/home/path/nuclei_project/nuclei_project/webview/nuclei_output_json/"
    command = ["nuclei" , "-l", "/home/path/nuclei_project/nuclei_project/webview/target/list_ip_external_" + tanggal + "_" + waktu + ".txt", "-c", "9", "-pc", "9", "-je", path_to_file + "nuclei_output_" + tanggal + "_" + waktu + ".json"]
    #print(command)
...
def sending_to_slack():
    ...
    notif_messages = f"Hi Infosec Team, automated scanning for today {tanggal} completed at {hours} and all scan result available to see at http://34.34.223.15:8080/"
#Send a message
...
def main():
    ...
    try:
        ...
        if not new_ips:
            print("All IP addresses are identic with precious IP Lists collection result, Nuclei Scan Aborted")
            client = WebClient(token=slack_token)
            client.chat_postMessage(
                channel="va-automation",
